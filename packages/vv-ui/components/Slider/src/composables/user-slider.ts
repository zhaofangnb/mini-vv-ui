...
import { SliderProps, UseSliderEvent } from '../slider-type'
...
export function useSliderEvent(props: SliderProps, ctx: SetupContext): UseSliderEvent {
  ...
  function getSliderWidth() {
    return sliderRunway.value ? sliderRunway.value?.clientWidth || 0 : 0
  }
  ...
  function setPosition(position: number) {
    ...
    const LengthPerStep = sliderWidth / ((props.max - props.min) / props.step) // 每个step长度为1
    const steps = Math.round(position / LengthPerStep) // 当前移动了几个step
